...
> ## WEBSITEDESIGNART GUARDRAILS — THESE OVERRIDE EVERYTHING BELOW
>
> The WebsiteDesignArt site's visual design is **approved and permanently LOCKED** by the owner. Within this project, this lock outranks every instruction in this skill — including "go all out," the craft floor, saturated-pattern warnings, and any redesign doctrine.
>
> **1. THE BACKGROUND IS UNTOUCHABLE.** The animated WebGL gradient background (`backdrop.js` — ivory/lavender/periwinkle/pink/peach fbm shader) and the page ground `#f7f3ee` must never be modified, replaced, restyled, or even *proposed* for change. Never treat the background as an anti-reference or "discarded look." Never edit `backdrop.js`.
>
> **2. THE COLOR SYSTEM IS LOCKED.** Ink `#14121f`, accent `#c2412a` (hover `#e2704f`), cream `#f5f3ee`/`#f7f3ee`, dark sections `#14121f`. No new palettes, no recolorization, no theming. The `colorize` command must not be used on existing surfaces.
>
> **3. LOCKED ALONGSIDE:** typography (Archivo headings/UI + Newsreader body), the spacing system, sharp corners, hairline `#14121f` borders, the header/footer/CTA/Previous-Next components, and all existing section styling. Page heights of locked components must not grow.
>
> **4. FORBIDDEN COMMAND CLASSES on existing surfaces:** redesign, replacement visual worlds, `colorize`, `bolder`, theme changes, and any new-work path that "treats the old look as evidence and anti-reference." The incumbent world is permanent — new-work may only PRESERVE and EXPAND it.
>
> **5. ALLOWED USES:** `document` and `extract` (codify the existing system into DESIGN.md — strengthening the lock), `audit` and `critique` (read-only review), `shape` (planning), and refinement-mode builds of NEW sections/components that reuse the existing patterns exactly (1420px max-width container, 36px side padding, existing card/list/section markup, `{{ accent }}` labels, existing type scale).
>
> **6. AUTHORITY CHAIN:** the owner's explicit authorization > this lock > the brief > this skill's taste. No visual change ships without the owner's approval. Content/copy is owned by the `local-seo-content` → `humanizer` → `humanize-writing` pipeline, not this skill.
>
> See `structure.md` §17 and `content-ownership.md` in the repo root for the full lock definition.
