...
> **WebsiteDesignArt project adaptations** (these override the generic rules below when they conflict):
> 1. Never invent opinions, personal experiences, anecdotes, or personality — the `local-seo-content` skill's HUMAN EDITORIAL AUDIT and anti-fabrication rules always win.
> 2. Verified business facts, keyword/topic ownership, internal-link purpose, and search intent must survive every rewrite untouched.
> 3. Pattern #14 (cut em dashes) is relaxed for this project: the site's approved voice deliberately uses em dashes (see existing hero/card copy). Fix genuine overuse within a paragraph, but do not strip them site-wide.
> 4. Source: https://github.com/blader/humanizer (MIT, v2.9.1).
